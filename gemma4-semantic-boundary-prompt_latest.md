...
Do not stop before an already-present negation, condition, number continuation, or necessary complement that belongs to the same thought. For example, do not treat the first clause of "It succeeds only if" as an unconditional complete statement.
...
A leading discourse marker attached to an unfinished explanation is not a separate complete answer just because it is followed by a comma. Keep it with the explanation unless context establishes an independent completed reply.
...
문서의 시스템 프롬프트를 그대로 추출해 현재 Mac에 로드된 Gemma 4 모델로 **18회 개발 회귀 검증**을 실행했다. 시스템 프롬프트 외의 API 어댑터·출력 계약·모델 설정은 변경하지 않았다.

- 기존 경계 사례 14개, 문서의 입력 예시 비강제/강제 2개, 추가 강제 배출 2개.
- **18/18 응답이 현재 서버의 JSON·선택 ID 계약을 통과했고, 수동 기대 경계와 일치했다.**
- 완성된 앞부분과 미완성 꼬리를 분리했고, 짧은 답·완성된 수량 표현·전치사로 끝나는 질문도 선택했다.
- 강제 배출 3건은 문장 완성 여부와 관계없이 마지막 ID까지 선택했다. 이는 자연스러운 경계의 성공이 아니라 기존 서버 계약 준수 확인이다.
- LAN 요청 시작부터 선택 완료까지 약 **226~592ms**였으며 18회 모두 1초 안에 응답했다. 전체 자막 지연이나 동시 번역 처리량을 측정한 값은 아니다.

기존에 검토한 문장과 문서 예시를 사용해 각 사례를 한 번씩 실행했다. 따라서 독립적인 정확도 평가나 전체 영상의 개선 보장이 아니다. 제품 코드와 실행 중인 MyVote 서버에는 아직 적용하지 않았다.

검증한 시스템 프롬프트 SHA-256:

```text
2e04a944d0de4b642acc5b9f5ce15c0e7b35b0bcffe22133c25822881b925fa8
```

[실제 호출·응답·소요 시간과 재현 자료](../artifacts/gemma4-boundary-prompt-validation-2026-09-16/README.md)
