...
class GatewayClient:
    ...
    async def open(self, *, source_language: str = "", target_language: str = "ko",
                   ...
                   capabilities: tuple[str, ...] = ("caption_groups_v1", "caption_refinement_v1", "semantic_translation_v1")) -> pb.ServerMessage:
        """Request caption_words_v1 explicitly for bounded committed-word evidence.

        Default desktop traffic does not request or receive word traces.
        """
        if self.channel is None or self.call is not None:
            raise RuntimeError("Open one stream inside the client context")
        if (not isinstance(capabilities, tuple) or len(capabilities) > 16
                or any(not isinstance(value, str) or not re.fullmatch(r"[a-z][a-z0-9_]{0,63}", value)
        ...
        negotiated = data.get("capabilities", [])
        if (not isinstance(negotiated, list) or len(negotiated) > 16
                or any(not isinstance(value, str) or value not in capabilities for value in negotiated)
                or len(set(negotiated)) != len(negotiated)):
            raise GatewayProtocolError("Invalid negotiated server capabilities")
