...
class EngineBundle:
    ...
    overlap_separator: Any = None
    overlap_config: Any = None
...
def validate_open(message: pb.ClientMessage) -> pb.OpenSession:
    ...
    if (len(opening.capabilities) > 16 or len(set(opening.capabilities)) != len(opening.capabilities)
            or any(not re.fullmatch(r"[a-z][a-z0-9_]{0,63}", value) for value in opening.capabilities)):
        raise GatewayFailure(grpc.StatusCode.INVALID_ARGUMENT, "invalid_capabilities")
    return opening
...
class GatewayService(rpc.SpeechGatewayServicer):
    ...
    async def StreamSession(self, request_iterator, context):
        ...
        def message(kind, data, segment_id=""):
            if kind == "caption.group_replaced" and "caption_refinement_v1" not in opening.capabilities:
                # Earlier group clients have strict nested field allowlists.
                data = {**data, "children": [
                    {key: ({k: v for k, v in value.items() if k != "result_revision"}
                           if key == "translation" and isinstance(value, dict) else value)
                     for key, value in child.items()
                     if key not in ("text_operation", "source_language")}
        ...
        async def process_input():
            ...
            try:
                ...
                capabilities = [value for value in opening.capabilities
                                if value in ("caption_groups_v1", "caption_words_v1", "caption_refinement_v1", "semantic_translation_v1")]
                if getattr(bundle.provider, "supports_semantic_translation", None) is False:
                    capabilities = [value for value in capabilities if value != "semantic_translation_v1"]
                separator = bundle.overlap_separator if "caption_groups_v1" in capabilities else None
                overlap_config = None
                if separator is not None:
                    from .overlap_pipeline import OverlapCaptionConfig
                    overlap_config = replace(bundle.overlap_config or OverlapCaptionConfig(),
                ...
                session = StreamingSession(opening.session_id, _FencedTranscriber(bundle.transcriber, fence),
                    ...
                    overlap_separator=separator, overlap_config=overlap_config, overlap_run_native=fence.run,
                    config=PipelineConfig(source_language=opening.source_language or None,
                        ...
                        emit_word_trace="caption_words_v1" in capabilities,
                        context_refinement="caption_refinement_v1" in capabilities,
                        semantic_translation="semantic_translation_v1" in capabilities,
                        context_refinement_event_bytes=min(60000, self.config.max_event_bytes),
                        word_trace_event_bytes=min(65536, self.config.max_event_bytes)))
                reblock = _Reblocker()
                ...
                started_data = {"protocol_version": 1, "engine_mode": bundle.engine_mode,
                    ...
                    "context_refinement_status": ("enabled" if "caption_refinement_v1" in capabilities
                                                  else "client_not_supported"),
                    "semantic_translation_status": ("enabled" if "semantic_translation_v1" in capabilities
                                                     else "provider_not_supported" if "semantic_translation_v1" in opening.capabilities
                                                     else "client_not_supported"),
                ...
                if "semantic_translation_v1" in capabilities:
                    orchestrated = getattr(bundle.provider, "semantic_mode", None) == "orchestrated"
                    started_data.update(semantic_min_request_interval_ms=800, semantic_max_hold_ms=4000,
                        semantic_request_timeout_ms=min(opening.translation_budget_ms, 2500) + (1000 if orchestrated else 0),
                        semantic_total_budget_ms=8000, semantic_total_budget_scope="semantic_stable_source_clock")
                    if orchestrated:
                        started_data["semantic_orchestrator_budget_ms"] = 1000
                started_data["overlap_status"] = ("client_not_supported" if bundle.overlap_separator is not None
                                                  and "caption_groups_v1" not in capabilities else session.overlap_status)
                if overlap_config is not None:
                    started_data["overlap_correction_budget_ms"] = overlap_config.correction_budget_ms
                    started_data["overlap_correction_timing_scope"] = INGRESS_SCOPE
                if self.config.heartbeat_interval_ms is not None:
                    started_data["heartbeat_interval_ms"] = self.config.heartbeat_interval_ms
                if bundle.speaker_analyzer is not None and hasattr(bundle.speaker_analyzer, "runtime_configuration"):
                    started_data["speaker_configuration"] = bundle.speaker_analyzer.runtime_configuration()
                ...
                while True:
                    ...
                    if item is _FINISH:
                        ...
                        await emit("session.completed", {"counts": terminal_counts(),
                            ...
                            "speaker_mapping_diagnostics": session.speaker_mapping_diagnostics,
                            "engine_revision": ENGINE_REVISION,
...
async def warmup_bundle(bundle: EngineBundle, *, translation_budget_s: float = 30.) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "overlap_warmup": "not_configured" if bundle.overlap_separator is None else "not_performed_no_preload_api",
    }
    started = time.monotonic()
    try:
        ...
        if bundle.overlap_separator is not None and hasattr(bundle.overlap_separator, "preload"):
            await fence.run(bundle.overlap_separator.preload)
            report["overlap_warmup"] = "weights_preloaded_no_quality_test"
...
def _translation_configuration(args):
    """Validate routing before model loading, sockets or inference start."""
    profile = getattr(args, "translation_profile", "generic")
    orchestrator = getattr(args, "orchestrator_model", None)
    explicit_models = getattr(args, "translation_model_id", None)
    explicit_workers = getattr(args, "translation_workers", None)
    if explicit_models is not None and (not isinstance(explicit_models, (list, tuple))
                                        or any(not isinstance(value, str) for value in explicit_models)):
        raise ValueError("translation-model-id must be a sequence of exact model IDs")
    models = tuple(explicit_models) if explicit_models else (args.model,)
    workers = explicit_workers if explicit_workers is not None else 4 if orchestrator else 1
    if profile not in ("generic", "translategemma"):
        raise ValueError("Unknown translation profile")
    if type(workers) is not int or not 1 <= workers <= 4:
        raise ValueError("translation-workers must be an integer in [1, 4]")
    if not 1 <= len(models) <= 4 or len(set(models)) != len(models) or len(models) > workers:
        raise ValueError("Use one shared translation model ID or up to four unique IDs within the worker limit")
    identifiers = models + ((orchestrator,) if orchestrator is not None else ())
    if any(not isinstance(value, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._/:@+\-]{0,255}", value)
           for value in identifiers):
        raise ValueError("Use exact model IDs without whitespace or control characters")
    if profile == "translategemma":
        if not orchestrator:
            raise ValueError("TranslateGemma requires --orchestrator-model for semantic decisions and same-language correction")
        if args.backend != "lmstudio":
            raise ValueError("The orchestrated TranslateGemma profile currently requires --backend lmstudio")
        if orchestrator in models:
            raise ValueError("The orchestrator model ID must differ from translation model IDs")
        if "translategemma" in re.sub(r"[^a-z0-9]", "", orchestrator.lower()):
            raise ValueError("Use a general instruction model, not TranslateGemma, as the orchestrator")
    else:
        if orchestrator is not None or explicit_models is not None or explicit_workers is not None:
            raise ValueError("Orchestrator and translation worker options require --translation-profile translategemma")
        if any("translategemma" in re.sub(r"[^a-z0-9]", "", model.lower()) for model in models):
            raise ValueError("TranslateGemma cannot use generic JSON/correction prompts; select --translation-profile translategemma and --orchestrator-model")
    return profile, orchestrator, models, workers


def _configured_translation_provider(client, args, *, max_tokens=None):
    profile, orchestrator, models, workers = _translation_configuration(args)
    if profile == "translategemma":
        from .orchestrated_translation import OrchestratedTranslationProvider
        return OrchestratedTranslationProvider(client, args.endpoint, orchestrator, models,
            translation_concurrency=workers,
            translation_max_tokens=512 if max_tokens is None else max_tokens,
            orchestrator_max_tokens=256)
    ...
    return provider_type(client, args.endpoint, args.model,
...
async def _serve(args):
    ...
    analyzer_type = LocalSpeakerAnalyzer
    analyzer_options = {}
    activity_policy = getattr(args, "experimental_speaker_activity", None)
    if activity_policy is not None:
        if activity_policy not in ("trusted", "singleton"):
            raise ValueError("Unknown speaker activity policy")
        if not args.speaker_segmentation_model:
            raise ValueError("Speaker activity projection requires both speaker model paths")
        if getattr(args, "experimental_speaker_pooling", False):
            raise ValueError("Choose activity projection or short speech pooling for a fixed comparison")
        from .speaker_activity import LocalSpeakerActivityAnalyzer
        analyzer_type = LocalSpeakerActivityAnalyzer
        analyzer_options["activity_policy"] = activity_policy
    if getattr(args, "experimental_speaker_pooling", False):
        if not args.speaker_segmentation_model:
            raise ValueError("Short speech pooling requires both speaker model paths")
        from .speaker_pooling import LocalSpeakerPoolingAnalyzer
        analyzer_type = LocalSpeakerPoolingAnalyzer
    overlap_model = getattr(args, "experimental_overlap_model", None)
    if overlap_model is not None and not args.speaker_segmentation_model:
        raise ValueError("Live overlap requires both speaker models")
    _translation_configuration(args)
    overlap_separator = None
    if overlap_model is not None:
        import torch
        # One startup setting shared by Torch CPU work, never changed per chunk.
        torch.set_num_threads(getattr(args, "overlap_threads", 1))
        from .separation import LocalConvTasNetSeparation
        from .overlap_separation import FrequencySpeechSeparation
        # The one-session gateway retains the audited weights between sessions.
        # Its native fence prevents reuse while an old call is still running.
        overlap_separator = FrequencySpeechSeparation(LocalConvTasNetSeparation(overlap_model))
    ...
    def factory(opening, *, max_tokens=None):
        ...
        return EngineBundle(MlxWhisperWindowTranscriber(args.asr_model, language=opening.source_language or None),
            _configured_translation_provider(client, args, max_tokens=max_tokens),
            SileroOnnxVad(args.vad_model), aclose=client.aclose, engine_mode="configured_local_engines",
            speaker_analyzer=analyzer_type.from_local_models(
            ...
            overlap_separator=overlap_separator)
...
def main():
    ...
    parser.add_argument("--orchestrator-model", help="Exact general instruction model ID for semantic decisions")
    parser.add_argument("--translation-workers", type=int, choices=range(1, 5),
                        help="Logical translation slots, default 4 with an orchestrator; not a model-copy count")
    parser.add_argument("--translation-model-id", action="append",
                        help="Exact translation model/instance ID; repeat for physical instances, otherwise --model is shared")
    ...
    parser.add_argument("--experimental-speaker-pooling", action="store_true",
                        help="Pool short clean speech from the current segmentation window; experimental, disabled by default")
    parser.add_argument("--experimental-speaker-activity", choices=("trusted", "singleton"),
                        help="Project independently learned identity onto current-window activity; research option, disabled by default")
    parser.add_argument("--experimental-overlap-model", type=Path,
                        help="Pinned ConvTasNet checkpoint for experimental live frequency separation and two captions")
    parser.add_argument("--overlap-threads", type=int, choices=range(1, 9), default=1,
                        help="Torch CPU threads fixed at startup when live overlap is enabled")
