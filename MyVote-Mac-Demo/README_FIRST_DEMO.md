...
- 같은 LAN 연결. 현재 서버 주소는 `192.168.219.103`이다. 공유기 주소 `192.168.219.1`과 다르다.
...
- 이번 준비 명령은 화자 특징 추출에 쓰는 Torch·TorchAudio를 각각 `2.11.0`으로 지정한다. Mac arm64용 Python 3.12·3.13 설치 파일의 존재를 확인했으며 실제 Mac 설치·import는 아직 확인 전이다.
...
현재 개인 묶음은 `192.168.219.103`용이다. 주소가 바뀐 상태에서 검증을 끄고 연결하지 않는다.
